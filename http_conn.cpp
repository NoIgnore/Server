...
http_conn::LINE_STATUS http_conn::parse_line()
{
    ...
    for (; m_checked_idx < m_read_idx; ++m_checked_idx)
    {
        ...
        else if (temp == '\n')
        {
            ...
            { // 如果1索引为\n，就算0索引为\r，组合成一个换行，那这个换行前面也没有任何数据。所以m_checked_idx 至少从2索引开始
...
http_conn::HTTP_CODE http_conn::parse_request_line(char *text)
{
    ...
    m_url = strpbrk(text, " \t"); // 判断第二个参数中的字符哪个在text中最先出现" "包含了(空格)和(\t不知道是啥玩意)
    if (!m_url)
    {
        return BAD_REQUEST;
    }
    // GET\0http://192.168.110.129:10000/index.html HTTP/1.1
    *m_url++ = '\0'; // 置位空字符，字符串结束符
    ...
    // http://192.168.110.129:10000/index.html HTTP/1.1
